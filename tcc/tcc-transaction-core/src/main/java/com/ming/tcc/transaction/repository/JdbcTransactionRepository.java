package com.ming.tcc.transaction.repository;

import com.ming.tcc.transaction.Transaction;
import com.ming.tcc.transaction.exception.TransactionIOException;
import com.ming.tcc.transaction.serializer.JdkSerializationSerializer;
import com.ming.tcc.transaction.serializer.ObjectSerializer;
import com.ming.tcc.transaction.utils.CollectionUtil;
...
public class JdbcTransactionRepository extends CachableTransactionRepository {
    ...
    static final Logger log = LoggerFactory.getLogger(JdbcTransactionRepository.class.getSimpleName());
    ...
    protected int doUpdate(Transaction transaction) {
        ...
        try {
            ...
            builder.append(StringUtil.isNotEmpty(domain) ? " AND DOMAIN = ?" : "");
    ...
    protected List<Transaction> doFindAllUnmodifiedSince(java.util.Date date) {
        ...
        try {
            ...
            builder.append(StringUtil.isNotEmpty(domain) ? ",DOMAIN" : "");
            // TRANSACTION_TYPE=1（主事务:1）
            ...
            transactions = getTransactions(stmt);
        } catch (Throwable e) {
            throw new TransactionIOException(e);
        } finally {
            closeStatement(stmt);
    ...
    protected List<Transaction> doFind(List<Xid> xids) {
        ...
        List<Transaction> transactions = new ArrayList<>();
        if (CollectionUtil.isEmpty(xids)) {
            return transactions;
        }
    ...
    private String getTableName() {
        return StringUtil.isNotEmpty(tbSuffix) ? "TCC_TRANSACTION" + tbSuffix : "TCC_TRANSACTION";
    }
}
